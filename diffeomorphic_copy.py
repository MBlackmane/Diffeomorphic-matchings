...
def fv_exp(x, y, sigma):
    return(torch.exp(-torch.norm(((x-y)/sigma))**2))
...
def grad_descent2(mu, nu, delta, n, sigmaI, sigmaV, sigmaR, alpha):
    ...
    while r <= n:     
        ...
        print(torch.norm(gradient_1 + (1/sigmaR**2)*gradient_2))
        alpha.grad.zero_()
        r += 1
    return(alpha)
...
def import_matrices(origin_name, target_name):
    # Import
    files = []
    for filename in (origin_name, target_name):
        # check for filename extension
        if not re.search(r'.*?\.mat$', filename):
            raise Exception('Wrong extension name {}, should be .mat'.format(filename))
        else:
            files.append(loadmat(filename))

    return files

@click.command()
@click.option('--origin', help='origin matrix file, (x,y)')
@click.option('--target', help='target matrix file, (x,y)')
...
def main(*args, **kwargs):
    x, x_4 = import_matrices(origin_name=kwargs['origin'], target_name=kwargs['target'])
    x1, y1 = [v for (k, v) in x.items() if not k.startswith('__')]
    ...
    mu = Mesure(torch.tensor(x4, dtype = torch.float32), 1*torch.rand(98))
    nu = Mesure(torch.tensor(y4, dtype = torch.float32), 10*torch.rand(98))
